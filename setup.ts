...
export default function ModelEngine() {
  const { vendors, setVendors, assumptions, setAssumption, tree, setTree, tam } = useModel();
  const [editorOpen, setEditorOpen] = useState(false);

  const updateVendor = (id: string, key: string, val: any) =>
    setVendors(vendors.map((v) => (v.id === id ? { ...v, [key]: val } : v)));

  const growth = assumptions.find((a) => a.name === "Growth Rate")?.value ?? 14;
  const penetration = assumptions.find((a) => a.name === "Penetration Rate")?.value ?? 60;
  const expansion = assumptions.find((a) => a.name === "Expansion Factor")?.value ?? 1.15;
      ...
      <div className="px-8 pt-6 pb-4 border-b bg-card">
        <div className="flex items-end justify-between">
          <div>
            ...
            <h1 className="text-2xl font-semibold text-mds-navy">Market Model Engine</h1>
          </div>
          ...
          <div className="p-5 space-y-5">
            ...
            <div className="rounded-lg border bg-card shadow-sm p-5">
              ...
              <div className="flex items-center gap-2 flex-wrap text-sm">
                ...
                <Op>×</Op>
                <FormulaBlock accent>Penetration⁻¹</FormulaBlock>
                <Op>×</Op>
                <FormulaBlock>(1 + Growth)</FormulaBlock>
                <Op>×</Op>
                <FormulaBlock accent>Expansion Factor</FormulaBlock>
                <Op>=</Op>
                <FormulaBlock total>Adjusted TAM</FormulaBlock>
              </div>
            ...
            <div className="rounded-lg border bg-mds-navy text-white shadow-sm p-6 flex items-center justify-between">
              <div>
                ...
                <div className="mt-2 inline-flex items-center gap-1 text-mds-blue-light text-sm">
                  <TrendingUp className="h-4 w-4" /> +{growth.toFixed(1)}% CAGR · Expansion {expansion.toFixed(2)}x
                </div>
              </div>
